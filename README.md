...
What happen if we try to "colored" the first 1000 digits of PI? (just in case, they looks like: `31415...a few more...2019`)
...
Oh! that looks prettier than the previous one, but did you see a pattern? At least for me, I can't get too much from that picture. So the question I'm asking now is, what happen if we try to encode other sequence or number? Let's try with the first 1000 pair numbers (`2,4,6,8,10,...,996,998,1000`) but, for practice reasons, let's concatenate them, in that case we obtain the following number `246810...9969981000`.
...
Amaizing, rihgt? I mean, it looks that there is a certain pattern, a much more complex one than the previous examples, but, seems like is there...Well tbh I'm not 100% convinved, but, for instance if we took the first 100 (is like doing some zoom over the previous image), we will notice that some patterns start in a certain region, and then appears in other regions with a different scale. And other stuff can be noticed too, but I prefer the reader makes their own analysis and take their own conclusions.
...
Anyway, I hope some mathematician reach this and give me some more insight on it! But in any case, I think that this is a nice example of how seeing things from a different perspective can help you or motivate new ideas.
I hope you enjoyed this little experiment as much as I did, and feel free to collaborate with the project, play with it, and share your results! Also if there are questions or request, feel free to create an issue (or PR) in the repository.
